...
exports.getUserFavorites = catchAsync(async (req, res, next) => {
    ...
    if(!doc) {
        return next(new ApiError("no favorite in here please add favorite to yours wishlist", 404));
    }
    res.status(200).json({
...
exports.deleteuerFavorites = catchAsync(async (req, res, next) => {
    ...
    if(!doc) {
        return next(new ApiError("no wishlist plaease add to wishlist then you can dlete", 404));
    }
    res.status(200).json({
        status: httpStataus.SUCCESS,
        data: {
